		...
		const struct audio_action *action)
{
	...
	case AUDIO_ACTION_VOL:
		source->volume = action->volume.vol; break;
	case AUDIO_ACTION_MUTE:
		source->muted = action->volume.set; break;
	case AUDIO_ACTION_PTT:
		source->push_to_talk_pressed = action->volume.set; break;
	case AUDIO_ACTION_PTM:
		source->push_to_mute_pressed = action->volume.set; break;
	}
}
